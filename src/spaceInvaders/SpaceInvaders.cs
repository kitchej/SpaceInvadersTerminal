...
namespace spaceInvaders{
    ...
    static class Program{
        ...
        static void Main(){
            ...
            // string base_path = "src\\spaceInvaders\\sprites";
            string base_path = "sprites";
            ...
            // Intro Screen
            screen.DrawSprite(new Sprite(Path.Join(new[] {base_path, "introGraphic.txt"}), 5, 5, "Header"));
            screen.ShowBuffer();
            Console.ReadKey();

            // Game Init
            Pawn ship = new Pawn(Path.Join(new[] {base_path, "ship.txt"}), startx: 30, starty: 36, spriteId: "ship", centralController: mainController, stackOrder: 0);
            Spawner shipProjectileSpawner = new Spawner(typeof(Pawn), 
                                                        new object[] { Path.Join(new[] {base_path, "projectile.txt"}), 30, 21, "projectile", mainController, 0, true}, 
                                                        typeof(ProjectileController), 
                                                        new object?[] {null, mainController, "ProjectileCon", 40}, 
                                                        mainController);

            Sprite livesLabel = new Sprite(Path.Join(new[] {base_path, "livesLabel.txt"}), startx: 1, starty: 1, spriteId: "livesLabel");
            Sprite life1 = new Sprite(Path.Join(new[] {base_path, "life.txt"}), startx: 7, starty: 1, spriteId: "life1", collisions: false);
            Sprite life2 = new Sprite(Path.Join(new[] {base_path, "life.txt"}), startx: 8, starty: 1, spriteId: "life2", collisions: false);
            Reset resetAction = new Reset(mainController, new Stack<Sprite>(new Sprite[]{life1, life2}), ship, input);

            Sprite northBounds = new Sprite(Path.Join(new[] {base_path, "horizBorder.txt"}), 1, 0, "northBounds");
            Sprite southBounds = new Sprite(Path.Join(new[] {base_path, "horizBorder.txt"}), 1, 39, "southBounds");
            Sprite westBounds = new Sprite(Path.Join(new[] {base_path, "vertiBorder.txt"}), 0, 1, "westBounds");
            Sprite eastBounds = new Sprite(Path.Join(new[] {base_path, "vertiBorder.txt"}), 59, 1, "eastBounds");
            ...
            MasterEnemyController masterEnemyController = new MasterEnemyController(mainController, 500, "MasterEnemyCon", new Spawner(typeof(Pawn), 
                                                                                                                        new object[] { Path.Join(new[] {base_path, "projectile.txt"}), 30, 21, "enemyProjectile", mainController, 0, true}, 
